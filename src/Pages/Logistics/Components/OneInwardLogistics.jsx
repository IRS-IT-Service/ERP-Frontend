...
const OneInwardLogistics = () => {
    ...
    if (oneLogisticsData?.status === "success") {
      ...
      const newRows = oneLogisticsData.data.NoOfBox.map((item, index) => {
        return {
          ...
          volWeight:
            oneLogisticsData?.data?.CourierType === "courier"
              ? ((item.length * item.width * item.height) / 5000).toFixed(2)
              : ((item.length * item.width * item.height) / 6000).toFixed(2),
  ...
  const columns = [
...

    {
      ...
      headerName: (
        <div>
          Dimensions (L x W x H)<sup>cm</sup>
        </div>
      ),
      flex: 0.2,
      width: 400,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography variant="body2" gutterBottom sx={{ fontWeight: "bold" }}>
          Courier-Type{" "}
          <StyleSpan>{oneLogisticsData?.data?.CourierType}</StyleSpan>
        </Typography>
